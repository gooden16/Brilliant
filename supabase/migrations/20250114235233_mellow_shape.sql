/*
  ...
  1. New Tables
    - canvas_products: Stores financial products associated with canvases
    - canvas_metrics: Stores metrics and KPIs for canvases
    - payment_methods: Stores available payment methods for canvases

  2. Changes
    - Add name and type columns to canvases table
    - Add new enums for canvas types, product types, and payment methods
...
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can view their own canvas products" ON canvas_products;
...
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can insert their own canvas products" ON canvas_products;
...
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can update their own canvas products" ON canvas_products;
...
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can view their own canvas metrics" ON canvas_metrics;
...
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can insert their own canvas metrics" ON canvas_metrics;
...
DO $$ BEGIN
    DROP POLICY IF EXISTS "Users can update their own canvas metrics" ON canvas_metrics;
