...
import { ArrowLeft, Building2, User, Building, Phone, MapPin, Wallet, ChevronDown, Check } from 'lucide-react';
...
type CounterpartyType = 'business' | 'individual' | 'external_account';
type PaymentMethodType = 'ach' | 'check' | 'zelle';
...
export default function CounterpartyForm({ onBack, onSubmit }: CounterpartyFormProps) {
  const [counterpartyType, setCounterpartyType] = useState<CounterpartyType>('business');
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h2 className="text-2xl font-playfair">Add New Counterparty</h2>
      </div>

      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="bg-navy/50 rounded-xl p-6 backdrop-blur-sm border border-white/5">
          <h3 className="font-playfair text-lg mb-6">Counterparty Information</h3>
          ...
          <div className="space-y-6">
            <div className="flex gap-4">
              <button
                type="button"
                onClick={() => {
                  setCounterpartyType('business');
                  setIsClientAccount(false);
                }}
                className={`flex-1 p-4 rounded-xl border-2 transition-colors ${
                  counterpartyType === 'business' && !isClientAccount
              ...
              <button
                ...
                className={`flex-1 p-4 rounded-xl border-2 transition-colors ${
                  counterpartyType === 'individual' && !isClientAccount
                    ? 'border-dusty-pink bg-white/5'
                    : 'border-white/10 hover:border-white/20'
                }`}
              >
              ...
              <button
                type="button"
                onClick={() => {
                  setCounterpartyType('external_account');
                  setIsClientAccount(true);
                }}
                className={`flex-1 p-4 rounded-xl border-2 transition-colors ${
                  counterpartyType === 'external_account' && isClientAccount
                    ? 'border-dusty-pink bg-white/5'
                    : 'border-white/10 hover:border-white/20'
                }`}
              >
                <Wallet className="w-6 h-6 mx-auto mb-2" />
                <div className="font-medium">My External Account</div>
              </button>
            </div>

            <div>
              <label className="block text-sm font-medium text-cream/80 mb-2">
                {counterpartyType === 'business' ? 'Business Name' : 
                 counterpartyType === 'individual' ? 'Full Name' : 
                 'Account Name'}
              </label>
              <input
                type="text"
                ...
                className="w-full px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-cream placeholder-cream/30 focus:ring-2 focus:ring-dusty-pink focus:border-transparent transition-colors"
                placeholder={
                  counterpartyType === 'business' ? 'Enter business name' : 
                  counterpartyType === 'individual' ? 'Enter full name' : 
